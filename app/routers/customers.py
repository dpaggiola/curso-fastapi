...
async def read_customer(customer_id: int, session: SessionDep):
  ...
  if not customer_db:
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Customer doesn't s")
  return customer_db
...
async def subscribe_customer_to_plan(customer_id: int, plan_id: int, session: SessionDep, plan_status: StatusEnum = Query()):
  ...
  if customer_db.id is None or plan_db.id is None:
    raise HTTPException(
      status_code=status.HTTP_404_NOT_FOUND,
      detail="The customer or plan doesn't exist",
    )
  
  customer_plan_db = CustomerPlan(id=None, plan_id=plan_db.id, customer_id=customer_db.id, status=plan_status)
